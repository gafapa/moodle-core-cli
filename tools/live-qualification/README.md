...
- `runner/run-qualification.mjs`: executes plan, apply, live verify, and an
  unchanged rerun for the four provider pairings with the public `moodlia`
  package, then verifies the MoodlIA-to-MoodlIA Page bytes and references. The
...
4. Install the package under test in `runner/` (`npm install moodlia@<version>`
