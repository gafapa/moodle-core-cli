...
const cliPath = path.join(runner, 'node_modules', 'moodlia', 'cli', 'moodlia.mjs');
...
for (const scenario of scenarios) {
  ...
  const common = ['--config', profilesPath, '--state', statePath, '--format', 'json'];
  ...
  job ??= invoke(scenario.name, 'apply', [
      'course', 'sync', '--apply-plan', planPath,
  ...
  verification ??= invoke(scenario.name, 'verify', [
    'course', 'sync', '--verify-plan', plan.plan_id,
    '--verify-job-id', job.job_id,
    ...common
  ]);
