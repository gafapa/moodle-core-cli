...
The preview synchronizes selected metadata and proven structure plus selected portable authoring through MoodlIA: Pages, Labels, URLs, resources, folders, Books, assignments and Workshop grading forms. Page and Book editor files are staged as one destination draft per owner, preserving nested paths and Unicode names. The capability matrix distinguishes create/update gaps. Learner submissions, grades, attempts, and historical completion remain excluded.
